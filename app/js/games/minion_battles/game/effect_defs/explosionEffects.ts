...
export const explosionEffectDef: IEffectDef = {
    ...
    updateVisual(visual: Container, effect: Effect, _context: IEffectRenderContext): void {
        ...
        if (props.direction === 'expand') {
            if (props.shape === 'ring') {
                const startRadius = 4;
                const radius = startRadius + (base - startRadius) * p;
                g.circle(0, 0, radius);
                g.stroke({ color, width: 3, alpha: Math.max(0, 0.9 * (1 - p)) });
                if (p > 0.15) {
                    const p2 = (p - 0.15) / 0.85;
                    const echoR = startRadius + (base - startRadius) * p2 * 0.85;
                    g.circle(0, 0, echoR);
                    g.stroke({ color, width: 2, alpha: Math.max(0, 0.45 * (1 - p2)) });
                }
            } else {
