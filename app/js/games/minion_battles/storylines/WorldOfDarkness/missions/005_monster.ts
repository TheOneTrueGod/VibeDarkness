...
const ENEMIES = [
    {
        ...
        /** Only this unit is player-controllable (not summoned pack wolves). */
        controlGroupId: 'boss',
    },
];
...
const LEVEL_EVENTS: LevelEvent[] = [
		{
			...
			maxUnits: 4,
			spawns: [{ characterId: 'slime', spawnBehaviour: 'closest', closestConfig: { inDarkness: true }, spawnCount: 1, unitAITreeId: 'hunt' }],
		},
    {
        type: 'victoryCheck',
...
export class MonsterMission extends BaseMissionDef {
    ...
    enemies = ENEMIES;
    // Match by controlGroupId only — Boss tag alone must not claim summoned pack wolves.
    playerControl = [{ controlGroupId: 'boss', label: 'Control Alpha Wolf' }];
