...
| **`desyncDebug-getLobbyLog`** | Filters `lobby_log.jsonl` by tick range, `--keyword` (repeatable), and/or `--severity <level>` (floor: log/info/warn/error/critical). Tick extraction checks `obj.tick`, `obj.gameTick`, `obj.context.engineTick`, `obj.context.tick`. | Use to surface desync/resync events, rejection reasons, and timestamps in the incident window. |
| **`desyncDebug-desyncs`** | Auto-detects fingerprint mismatch events in `lobby_log.jsonl` and bundles nearby fingerprints, orders, snapshots, and suggested next commands for each one. Supports `--window <ticks>` (default 20). | **Start here for any desync investigation when user_state logs are unavailable** — replaces the four-step manual discovery sequence. |
| **`desyncDebug-diffSnapshot`** | Diffs a host `snapshots/<tick>.json` against a client state from `--vs <file>` or `--vs-log <tick>` (searches `lobby_log.jsonl` for an embedded state dump). Shows key sync fields (randomSeed, gameTick) and all diverging top-level fields. Use `--field <dotpath>` to zoom in. | Use when fingerprints differ but you suspect game states actually agree (phantom desync). Answers "did the serialized states match?" without needing user_state logs. |
...
4. Only if snapshots are missing or insufficient: run **`desyncDebug-desyncs`** first — it auto-detects all fingerprint mismatch events and bundles their surrounding context in one command. Then fall back to **`fingerprints.jsonl`**, **`orders.jsonl`**, and **`lobby_log.jsonl`** individually only if the bundle output is insufficient.
   - If fingerprints diverge but you suspect states agree (phantom desync): run **`desyncDebug-diffSnapshot -- --lobby <CODE> --tick <snap> --vs-log <desync_tick>`** to confirm whether `randomSeed` and other state fields actually match.
